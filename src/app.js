...
// ═══════════════════════════════════════════════════════════════════════════
// MEDIA MANAGEMENT (FIXED EJECT + PROTECTED PURGE)
// ═══════════════════════════════════════════════════════════════════════════

function ejectCurrent() {
    if (APP.media.queue.length === 0 || APP.media.currentIndex === -1) return;
    
    log(`EJECTING: ${APP.media.queue[APP.media.currentIndex].name}`);
    ...
    // 3. Handle Empty State vs Next Item
    if (APP.media.queue.length === 0) {
        ...
        // Move to previous index so rotateMedia picks up the "new" current
        APP.media.currentIndex = (APP.media.currentIndex - 1 + APP.media.queue.length) % APP.media.queue.length;
        rotateMedia(); 
    }
...
function purgeAll() {
    ...
    // NUCLEAR SHIELD: First click arms, second click fires
    if (!btn.classList.contains('armed')) {
        btn.classList.add('armed');
        btn.textContent = 'CONFIRM PURGE?';
        log('SYSTEM: PURGE_ARMED - CLICK AGAIN TO WIPE');
        
        // Auto-disarm after 3 seconds if not clicked
        setTimeout(() => {
    ...
    // If already armed, proceed with total wipe
    btn.classList.remove('armed');
...
   ═══════════════════════════════════════════════════════════════ */
/* ═══════════════════════════════════════════════════════════════
   IGNITION: DOM READY & SAFE HANDSHAKE (FINAL STABLE)
   ═══════════════════════════════════════════════════════════════ */
/* ═══════════════════════════════════════════════════════════════
   IGNITION: DOM READY & SAFE HANDSHAKE (PRO EDITION)
   ═══════════════════════════════════════════════════════════════ */
/* ═══════════════════════════════════════════════════════════════
   IGNITION: DOM READY & SAFE HANDSHAKE (PRO EDITION)
   ═══════════════════════════════════════════════════════════════ */
...
function triggerSeismic() {
    log('SYSTEM: SEISMIC_IMPACT');
    document.body.classList.remove('bass-hit');
    void document.body.offsetWidth; // Force CSS reflow
    document.body.classList.add('bass-hit');
    setTimeout(() => document.body.classList.remove('bass-hit'), 150);
}
...
function updateVU() {
    ...
    if (APP.vj.rumbleEnabled && bassDelta > 50 && currentBass > 180) {
        triggerSeismic();
    }
    APP.vj.lastBassLevel = currentBass;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const ACTION_MAP = {
        ...
        'btn-prev': previousMedia,
        'btn-cycle': toggleCycle, 
        'btn-eject': ejectCurrent,
        
        // Audio Controls
        'btn-audio': () => $('file-audio').click(),
        'btn-stop': stopAudio,
        'btn-next-track': playTrack,
        'btn-stereo': () => setSpatialMode('stereo'),
        'btn-spatial': () => setSpatialMode('3d'),
        'btn-dolby': () => setSpatialMode('dolby'),

        // Lower Thirds
        'btn-lt-guest': () => showLowerThird('guest'),
        'btn-lt-track': () => showLowerThird('track'),
        'btn-lt-breaking': () => showLowerThird('breaking'),
        'btn-lt-off': hideLowerThird,

        // Station Bug
        'btn-upload-logo': () => $('file-logo').click(),
        'btn-clear-logo': () => { APP.bug.image = null; updateBug(); },
        'btn-bug-toggle': toggleBug,

        // System Actions
        'btn-rumble': triggerSeismic, 
        'btn-ui-react': (e) => { APP.vj.uiReactivity = !APP.vj.uiReactivity; e.target.classList.toggle('on', APP.vj.uiReactivity); },
        'btn-master-reset': masterReset, 
        'btn-purge': purgeAll, 
        'btn-fs': toggleFullscreen,
        'btn-vhs': toggleVHS,
        'btn-crt': toggleCRT,
        'btn-projector': openProjector,
        'btn-capture30': capture30s,
        'btn-export-dna': exportDNA,
        'btn-enter-vr': enterVR,

        // Impact FX
        'btn-stutter': impactStutter,
        'btn-invert': impactInvert,
        'btn-crush': impactCrush
    };
    // Update Station Bug Text & Style dynamically
    $('bug-text').oninput = (e) => { APP.bug.text = e.target.value; updateBug(); };
    $('bug-style-select').onchange = (e) => { APP.bug.style = e.target.value; updateBug(); };

    // Update Lower Third Text inputs
    $('lt-title').oninput = (e) => { if(APP.lowerThird.visible) showLowerThird(APP.lowerThird.preset); };
    $('lt-sub').oninput = (e) => { if(APP.lowerThird.visible) showLowerThird(APP.lowerThird.preset); };

    // Set Audio Output
    $('audio-output').onchange = (e) => setAudioOutput(e.target.value);

    // Theme Picker Handshake
    document.querySelectorAll('.pal').forEach(pal => {
        pal.onclick = () => setTheme(pal.dataset.t);
    });
    ...
    // SLIDER HANDSHAKE
    const bindSlider = (id, prop, isPercentage = true) => {
        const el = $(id);
        if (!el) return;
        el.oninput = (e) => {
            const val = parseFloat(e.target.value);
            APP.vj[prop] = isPercentage ? val / 100 : val;
            const labelId = 'val-' + id.split('-')[1];
            if ($(labelId)) $(labelId).textContent = isPercentage ? val + '%' : (prop === 'hue' ? val + '°' : val);
        };
    };

    bindSlider('sl-b', 'brightness');
    bindSlider('sl-c', 'contrast');
    bindSlider('sl-s', 'saturation');
    bindSlider('sl-h', 'hue', false);
    bindSlider('sl-trail', 'trailAlpha');
    bindSlider('sl-rgb', 'rgbIntensity', false);
    bindSlider('sl-pix', 'pixelSize', false);
...
window.addEventListener('keydown', (e) => {
    if (e.target.tagName === 'INPUT') return; // Don't trigger if typing in a box

    switch(e.key.toLowerCase()) {
        case ' ': rotateMedia(); break;        // Space = Next Media
        case 'b': toggleBug(); break;          // B = Toggle Bug
        case 'h': toggleFullscreen(); break;   // H = Fullscreen
        case '1': impactStutter(); break;      // 1-3 = Impact FX
        case '2': impactInvert(); break;
        case '3': impactCrush(); break;
        case 'escape': masterReset(); break;   // ESC = Master Reset
    }
});
